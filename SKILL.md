...
2. Continue within the same section in displayed order: promotion product, applicable product/goal selection, and product assignment. In `选择转化`, first open and commit the approved `优化目标`; where the version exposes `更多目标`, search and select `表单预约`. Do not set conversion allocation or inspect a conversion-name row until the selected optimization goal is visibly committed. Next select `转化分配规则` = `全部相同` and wait until that rule is visibly committed **before** inspecting or checking any conversion name. Only then inspect the conversion list returned for the selected accounts: do not hardcode a conversion name such as `优投-表单预约`; when exactly one selectable conversion is returned, select that unique option and verify it corresponds to the chosen optimization goal.
...
Use `表单预约` only when it is available under the current account set and is the approved input. Do not assume that `更多目标` exists: open the goal control, inspect its visible choices, use `更多目标` only when the control is actually present, and otherwise choose the only visible approved option. The non-negotiable order inside `选择转化` is: (1) select and prove the optimization goal, (2) select and prove allocation `全部相同`, then (3) select the eligible conversion name. Never move step 2 or 3 ahead of the visible committed state of its predecessor.
...
- The conversion executor must not require the literal name `优投-表单预约`. Only the selected optimization goal is stable; inside `选择转化`, it must first select and verify the approved optimization goal, then select and verify conversion allocation `全部相同`, then select and verify the sole available conversion option returned for the current account set. Any executor path that configures allocation before the goal is visibly committed, checks a conversion-name row before allocation is visibly committed, or hard-codes a conversion name is a defect.
